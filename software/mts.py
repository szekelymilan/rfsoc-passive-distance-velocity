...
class mtsOverlay(Overlay):
    ...
    def __init__(self, bitfile_name='mts.bit', **kwargs):
        ...
        self.adc_capture_chA_I = self.memdict_to_view("hier_adc0_i_cap/axi_bram_ctrl_0")
        self.adc_capture_chA_Q = self.memdict_to_view("hier_adc0_q_cap/axi_bram_ctrl_0")
        ...
        self.adc_dma_I = self.deepCapture_I.axi_dma_adc
        self.adc_dma_Q = self.deepCapture_Q.axi_dma_adc
        self.adc_dram_capture = self.memdict_to_view("ddr4_0")
    ...
    def internal_capture(self, buffer):
        """ Captures ADC samples and stores to internal memories """
        if not np.issubdtype(buffer.dtype, np.int16):
            raise Exception("buffer not defined or np.int16!")
        if not buffer.shape[0] == 2:
            raise Exception("buffer must be of shape(2, N)!")

        self.trigger_capture()
        buffer[0] = np.copy(self.adc_capture_chA_I[0:len(buffer[0])])
        buffer[1] = np.copy(self.adc_capture_chA_Q[0:len(buffer[1])])

    def dram_capture(self, bufferI, bufferQ):
        """ Captures ADC samples to the PL-DRAM memory notebook provided buffer """
        if type(bufferI) != pynq.buffer.PynqBuffer or type(bufferQ) != pynq.buffer.PynqBuffer:
            raise Exception("A PYNQ allocated buffer is required!")

        if not np.issubdtype(bufferI.dtype, np.int16) or not np.issubdtype(bufferQ.dtype, np.int16):
            raise Exception("buffer not defined or np.int16")

        self.dac_enable.on()

        self.adc_dma_I.register_map.S2MM_DMACR.Reset = 1
        self.adc_dma_I.recvchannel.stop()
        self.adc_dma_Q.register_map.S2MM_DMACR.Reset = 1
        self.adc_dma_Q.recvchannel.stop()
        while int(self.adc_dma_I.register_map.S2MM_DMACR.Reset) == 1 or int(self.adc_dma_Q.register_map.S2MM_DMACR.Reset) == 1:
            pass # wait for reset to complete

        self.fifo_flush.off() # clear FIFO

        # because TLAST is not used, we must soft-reset the S2MM/recvchannel
        self.adc_dma_I.recvchannel.start()
        self.adc_dma_I.recvchannel.transfer(bufferI)
        self.adc_dma_Q.recvchannel.start()
        self.adc_dma_Q.recvchannel.transfer(bufferQ)
